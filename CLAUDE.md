...
MacBook Pro（現メイン機 / 唯一の稼働機）の bootstrap + 開発環境宣言ファイル + dotfiles を git 管理するリポ。Codex が主エージェントで、共有 Agent Core の正本は `~/src/claude-config`。本リポは新規Macへその正本を導入し、安全な自動キャッチアップを登録する役に徹する。元は Mac Studio ⇔ MBP の両機共通設定として作ったが、Mac Studio は **2026-06 引退**。
...
| `tools/bootstrap-agent-core.sh` | `claude-config` を clone/update して Codex / Claude / LM Studio 共通基盤を導入 |
...
| `bash tools/bootstrap-agent-core.sh` | Agent Coreだけを安全に導入・更新。以後は login 時 + 6時間ごとの LaunchAgent が追従 |
...
- Agent Coreの正本は `claude-config`。LaunchAgentが login 時 + 6時間ごとに origin/main を確認し、cleanなmainへ fast-forwardできる場合だけ自動追従する。旧・両機同期 (Syncthing / memory-sync.sh / `/sync*`) は 2026-05-09 全廃のまま維持
