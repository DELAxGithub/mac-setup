...
- **Target**: Mac Studio
- **Completed**: 2026-04-25 Mac Studio
...
- [x] `cd ~/src/claude-config && git pull` で `gcp-usage.md` が手元に来る
- [x] `/gcp-usage` を叩いて 4月の使用量サマリが表示される
...
### 2026-04-25 Mac Studio 動作確認

- `claude-config` pull 済み（5d57f34 取り込み）。`commands/gcp-usage.md` 配置確認。
- `~/.claude/commands/gcp-usage.md` は実体ファイル（symlink ではなかった）。問題なくスキル一覧に `gcp-usage` が出現、起動可能。
- `bq ls delax-budget:billing_export` → 空。BigQuery export はまだ反映なし（想定通り、明日以降）。
- スキル仕様通り Monitoring API にフォールバックして取得：

  | サービス | プロジェクト | 4月リクエスト数（〜04-25） |
  |---|---|---|
  | Gemini API | gen-lang-client-0728638538 | **2,723** |
  | Cloud TTS  | texttospeech-392819        | **2,691** |

  Gemini はサブ機計測の 2,722 とほぼ一致。TTS は月途中（5,138 は月末予測か別計測）。

- 残: 明日以降 `bq ls delax-budget:billing_export` で `gcp_billing_export_v1_010955_7D42FC_893DCB` が出現したら $ 表示確認。
