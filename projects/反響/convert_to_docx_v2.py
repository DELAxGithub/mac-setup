...
html = ['<html><head><meta charset="utf-8">',
        ...
        'a { color: #1155cc; text-decoration: underline; }</style>',
        '</head><body>']
in_table = False
...
for line in lines:
    ...
    if line.startswith('## '):
        ...
        row = '<tr>'
        for p in parts:
            p = re.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'<a href="\2" style="color: #1155cc; text-decoration: underline;">\1</a>', p)
            row += f'<td>{p}</td>'
        row += '</tr>'
        html.append(row)
    else:
...
tmp_html = '/Users/delaxpro/src/反響/report_styled.html'
